...
<script lang="ts" setup>
	...
	const toolbarOptions = reactive({
		...
		breadcrumbs: [
			...
			{
				title: `${filmForm.value?.name}: ${t("nav.edit")} `,
				href: "/films",
			},
		],
	});
	...
	watch (filmForm as Ref<Film>, (newVal) => {
		poster.value = newVal?.preview;
		toolbarOptions.breadcrumbs = [
			{
				title: `${newVal?.name}: ${t("pages.films.details")} `,
				href: `/films/${newVal?.id}`,
			},
			{
				title: t("nav.edit"),
				href: "/films",
